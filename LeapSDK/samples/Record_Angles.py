...
class SampleListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        for hand in frame.hands:
            ...
            if frameCounter > 0:
                writeToFile(thumbAngle,indexAngle,middleAngle,ringAngle,pinkyAngle)
                frameCounter -= 1
                print frameCounter
...
def writeToFile(thumb,index,middle,ring,pinky):
    angles = [thumb,index,middle,ring,pinky]
    with open('AsaveData', 'a+') as myfile:
...
def getHandAngles(hand):
    ...
    thumb_direction = [float(temp[37]),float(temp[38]),float(temp[39])]
