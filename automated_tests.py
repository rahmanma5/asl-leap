...
correctDict = {}
for i in range(0,26):
    correctDict[chr(ord('A')+i)] = 0

prefixed = [filename for filename in os.listdir('.') if filename.startswith("TESTINGHAND")]
print prefixed
for filename in prefixed:
    ...
    for i in range(0,26):
        ...
        letterInput = line.split(",")
        letterInput = np.reshape(letterInput,(1,-1))
        prediction = knn.predict(letterInput)
        if prediction[0] == chr(ord('A')+i):
            right += 1
            correctDict[chr(ord('A')+i)] += 1
            print chr(ord('A')+i) + " match"
        else:
            print chr(ord('A')+i) + " no match: " + prediction[0]
    
    print "Num right: " + str(right)
    file1.close() 

print "Final scores:"
correctAttempts = 0
totalAttempts = len(prefixed) * 26
for i in range(0,26):
    correctAttempts += correctDict[chr(ord('A')+i)]
    score = float(correctDict[chr(ord('A')+i)]) / float(len(prefixed))
    print chr(ord('A')+i) + ": " + str(score)
print str(correctAttempts) + "/" + str(totalAttempts) + " : " + str(float(correctAttempts)/float(totalAttempts)*100) + "%"
